...
class PersistentLoginTests(unittest.TestCase):
    ...
    def test_admin_assigns_hr_role_and_hr_sees_team_sections(self):
        ...
        self.assertEqual({tab.label for tab in hr_app.tabs}, {"Заявки и решения", "Назначить обучение", "Оценка сотрудника", "Обзор команды"})
    ...
    def test_approved_learning_and_binding_survive_new_app_session(self):
        admin_app = self.bootstrap()
        service = AuthService(self.auth_path)
        ...
        hr_email = 'completion-hr@local.example'
        hr_user = service.register('Проверяющий HR', hr_email, PASSWORD, PASSWORD)
        self.configure_in_ui(admin_app, hr_user['user_id'], role='hr')
        ...
        skill_id = next(skill['skill_id'] for skill in before['skills'] if skill['gap'] > 0)
        self.field(employee_app, 'Название своего курса').set_value('Проверочное обучение с подтверждением HR')
        self.field(employee_app, 'HTTPS-ссылка на курс').set_value('https://example.org/courses/persisted-learning')
        employee_app.text_area(key='custom-E0002-reason').set_value('Развитие навыка для следующего грейда')
        self.button(employee_app, 'Предложить курс HR').click().run()
        self.clean(employee_app)
        hr_app = self.login(self.app(), hr_email, 'hr')
        hr_app.session_state['hr_tab'] = 'Заявки и решения'
        hr_app.run()
        self.button(hr_app, 'Отправить решение сотруднику').click().run()
        self.clean(hr_app)
        employee_app.session_state['employee_tab'] = 'Мой маршрут'
        employee_app.run()
        self.clean(employee_app)
        employee_app.session_state['employee_tab'] = 'Мой маршрут'
        self.button(employee_app, 'Начать обучение').click().run()
        employee_app.session_state['employee_tab'] = 'Мой маршрут'
        self.button(employee_app, 'Завершить обучение').click().run()
        self.field(employee_app, 'Ссылка или номер сертификата').set_value('PERSISTED-LEARNING-CERTIFICATE')
        next(widget for widget in employee_app.multiselect if widget.label == 'Какие навыки развивали').set_value([skill_id])
        employee_app.session_state['employee_tab'] = 'Мой маршрут'
        self.button(employee_app, 'Отправить сертификат HR').click().run()
        self.clean(employee_app)
        pending = deepcopy(employee_app.session_state['views'][(employee_app.session_state['revision'], 'E0002')])
        self.assertEqual(pending['trajectory'], before['trajectory'])
        hr_app.run()
        self.button(hr_app, 'Подтвердить сертификат').click().run()
        self.clean(hr_app)
        employee_app.run()
        self.clean(employee_app)
