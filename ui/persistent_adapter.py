...
class PersistentAdapter(CoreAdapter):
    ...
    def get_employee_view(self, dataset, employee_id, approved=False):
        ...
        return super().get_employee_view(dataset, employee_id, approved=approved)
...
class AuthorizedGrowth:
    ...
    def _latest(self, employee_id, roles=None):
        ...
        dataset, self.adapter.dataset_revision = self.adapter.dataset_store.read()
        return dataset

    def approved_dataset(self, dataset):
        latest = self._latest(None, {'hr', 'admin'})
        return self.raw.approved_dataset(latest)
    ...
    def choose_course(self, dataset, employee_id, plan_id, course_id):
        latest = self._latest(employee_id, {'employee'})
        return self.raw.choose_course(latest, employee_id, plan_id, course_id)

    def hide_course(self, dataset, employee_id, plan_id, course_id, hidden=True):
        latest = self._latest(employee_id, {'employee'})
        return self.raw.hide_course(latest, employee_id, plan_id, course_id, hidden=hidden)

    def add_custom_course(self, dataset, employee_id, title, url, reason, provider='Предложение сотрудника'):
        latest = self._latest(employee_id, {'employee'})
        return self.raw.add_custom_course(latest, employee_id, title, url, reason, provider=provider)

    def cancel_training(self, dataset, employee_id, request_id):
        latest = self._latest(employee_id, {'employee'})
        return self.raw.cancel_training(latest, employee_id, request_id)

    def start_training(self, dataset, employee_id, request_id):
        latest = self._latest(employee_id, {'employee'})
        return self.raw.start_training(latest, employee_id, request_id)

    def submit_training_completion(self, dataset, employee_id, request_id, completed_on, evidence, skill_ids, tags=''):
        latest = self._latest(employee_id, {'employee'})
        return self.raw.submit_training_completion(latest, employee_id, request_id, completed_on, evidence, skill_ids, tags)
    ...
    def review_certificate(self, dataset, certificate_id, approve, awards=None, reason='', actor='employee'):
        ...
        latest, self.adapter.dataset_revision = self.adapter.dataset_store.read()
